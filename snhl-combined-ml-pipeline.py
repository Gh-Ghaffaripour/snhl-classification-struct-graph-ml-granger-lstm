...
CORR_FIG_SELECTED = os.path.join(OUTPUT_DIR, "corr_combined_selected.png")
DOC_MODEL_RESULTS = os.path.join(
    OUTPUT_DIR, "model_results_Combined_LeakFree.docx"
)
...
def main():
    ...
    df_whole = pd.read_csv(WHOLE_CSV)

    df = pd.concat([df_whole, df_struct], axis=1)
   ...
   #
    ...
    'clf__learning_rate': Real(0.05, 0.15, prior='log-uniform'),
    'clf__max_depth': Integer(2, 5),
    'clf__subsample': Real(0.5, 0.8),
    'clf__colsample_bytree': Real(0.5, 0.8),
    'clf__gamma': Real(0.5, 10.0, prior='log-uniform'),
    'clf__reg_alpha': Real(1.0, 15.0, prior='log-uniform'),
    'clf__reg_lambda': Real(5.0, 25.0, prior='log-uniform'),
